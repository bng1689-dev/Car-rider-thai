...
1. **อัปเดต `checked_at`** — ข้อมูลเกือบทั้งหมดลงวันที่ 2025-09-15
...
5. **เติมจำนวนประตู** — ยังไม่ทราบทั้ง 119 รุ่น เกณฑ์ `doors_in` จึงตรวจไม่ได้
...
├── vehicles.jsonl              L1 — 119 แถว (117 จากข้อมูลเดิม + 2 ที่เพิ่มภายหลัง · 1 บรรทัด = 1 รถ)
└── eligibility.jsonl           L2 — 205 แถว (1 บรรทัด = 1 ข้อกล่าวอ้าง)
...
└── render/{xlsx,html,pdf,csvx,webapp}.py

tools/
├── migrate_legacy.py        แปลงข้อมูลเดิม → L1+L2 (รันซ้ำได้ · ไม่ยอมเขียนทับข้อมูลที่เพิ่มภายหลัง)
├── diff_legacy.py           เทียบผลลัพธ์ 1,170 ช่องกับข้อมูลเดิม
└── import_verifications.py  นำผลตรวจจากเว็บแอปเข้า L2 (ตรวจทั้งชุดก่อนเขียน)

tests/            111 เทสต์ (unittest ของ stdlib ไม่ต้องลงอะไรเพิ่ม)
templates/        webapp.html · search.html · analysis_body.html
...
| รายการ | ผล |
|---|---|
| เทสต์ทั้งหมด | 111/111 ผ่าน |
| การถดถอยจากข้อมูลเดิม (1,170 ช่อง) | **0** |
| ข้อผิดพลาดจาก `validate` | 0 |
| output ที่สร้างได้ | 5/5 |
| หน้าค้นหาใน Chromium | 119 การ์ด · ไม่มี JS error · ค้นหา/กรองทำงานครบ |
...
แอปเป็น **ตัวเรนเดอร์อีกตัวหนึ่ง** ของชั้น L4 (`evbuild/render/webapp.py`) ไม่ใช่ระบบแยก —
คิวงานตรวจสอบ ช่องว่างการสำรวจ และเหตุผลของทุกข้อสรุปคำนวณฝั่ง Python แล้วฝังให้แอป
เป็นข้อมูล แอปจึงตอบไม่ตรงกับ report / xlsx / pdf ไม่ได้

สิ่งเดียวที่แอปเพิ่มเข้ามาคือ **ช่องทางให้คนบันทึกผลตรวจ** ซึ่งปิดวงจรที่ระบบเดิมไม่มี:

```
แอป: คนตรวจกับรายการจริง → บันทึกผล (db ของแอป)
  → ส่งออก JSON → tools/import_verifications.py (ตรวจด้วยกติกาเดียวกับ validate)
  → data/eligibility.jsonl → evbuild build → แอปรุ่นใหม่
```

ผลที่บันทึกในแอปแสดงทับข้อมูลในตาราง พร้อมเครื่องหมาย "รอนำเข้า" แต่ **ไม่ถูกนับ**
ในหน้าภาพรวมจนกว่าจะนำเข้าไฟล์ข้อมูล — เพื่อไม่ให้ข้อมูลที่ยังไม่ผ่าน validate
ปนกับข้อมูลที่ผ่านแล้ว ฟอร์มในแอปบังคับกติกาเดียวกับ pipeline: ผลจากรายการทางการ
ต้องมีลิงก์ที่มาและวันที่ ไม่งั้นบันทึกไม่ได้

ชุดสีของกราฟ: "ผ่าน" กับ "ไม่ผ่าน" เป็นน้ำเงินสองระดับ (ทั้งคู่คือ "รู้คำตอบแล้ว")
ส่วน "รอตรวจ" เป็นอำพัน (คือ "ยังไม่รู้") — การเข้ารหัสสีเองบอกหลัก "ไม่รู้ ≠ ไม่ผ่าน"
ผ่านตัวตรวจการแยกสีสำหรับผู้ตาบอดสีทั้งโหมดสว่างและมืด (ชุดเขียว/อำพันแบบทั่วไป
ตกการตรวจในโหมดมืด ΔE 4.6)

### 6.8 จุดที่ปรับพฤติกรรมได้
...
### 6.9 เพิ่มรุ่นรถหลังย้ายระบบ — Jaecoo 6 EV และ 6T EV

ข้อมูลเดิมมี Jaecoo รุ่นเดียว (5 EV) แต่หน้ารุ่นทางการของ Omoda & Jaecoo Thailand
มีรถไฟฟ้าล้วนอีก 2 รุ่นที่ขายแล้ว — **6 EV** (ขายตั้งแต่ปี 2567 ก่อนวันสำรวจเดิมด้วยซ้ำ)
และ **6T EV** · ส่วน 5 EV Ultra เป็นรุ่นย่อยของ 5 EV · 6T REEV (เครื่องยนต์ปั่นไฟ)
และ 7 SHS (ปลั๊กอินไฮบริด) อยู่นอกขอบเขตชุดข้อมูลที่เก็บเฉพาะ BEV

**ก. การเพิ่มรถเผยช่องโหว่ของ `surveyed`**
`resolve_cell` ตีความ "ไม่มีข้อกล่าวอ้าง" ในหมวดที่สำรวจครบแล้วว่า "ไม่ผ่าน" โดยไม่ดูว่า
การสำรวจครั้งนั้นเคยเห็นรถคันนี้หรือไม่ — ถ้าเพิ่มรถตรง ๆ ทั้งสองรุ่นจะขึ้น "ไม่ผ่าน"
8 ช่อง (Grab Premium · Exec · Bolt Comfort · XL) ทั้งที่ไม่เคยมีใครตรวจ ซึ่งคือ `-`
ของระบบเดิมกลับมาอีกครั้ง

แก้ด้วยฟิลด์ `added_at` (วันที่เพิ่มรุ่นเข้าชุดข้อมูล — บังคับ) และ
`resolve.covered_by_survey()` — การสำรวจที่เสร็จ ณ `as_of` พูดแทนได้เฉพาะรถที่
`added_at ≤ as_of` · รถที่เพิ่มทีหลังได้ "รอตรวจ" พร้อมเหตุผลว่าเพิ่มเมื่อไร และเข้า
คิวงานเอง · ไม่รู้วันที่เพิ่ม = ถือว่าไม่ครอบคลุม · รถจากข้อมูลเดิมทั้ง 117 รุ่นได้
`added_at: 2025-09-15` ผลเดิมทุกช่องจึงไม่เปลี่ยน

**ข. ตรวจกับรายการของ Grab (อัปเดต ก.ค. 69) แล้วนำเข้าผ่าน `import_verifications.py`**

| รุ่น | Grab SUV | Grab Premium |
|---|---|---|
| Jaecoo 5 EV | ผ่าน — รายการระบุ "Jaecoo 5" (เดิม: รอตรวจ) | ไม่ผ่าน — ไม่มียี่ห้อ Jaecoo ในรายการ |
| Jaecoo 6 EV | ผ่าน — รายการระบุ "Jaecoo 6" | ไม่ผ่าน |
| Jaecoo 6T EV | รอตรวจ — รายการระบุ "Jaecoo 6" ไม่ระบุ 6T | ไม่ผ่าน |

ทั้ง 6 แถวมี `source_url` · `checked_at: 2026-09-23` · `checked_by: claude-code`
ส่วน Grab Exec และ Bolt (ไม่เปิดเผยรายการ) ยังอยู่ในคิวงานตรวจสอบ

**ค. ด่านเทียบข้อมูลเดิมแยก "อัปเดต" ออกจาก "ถดถอย"**
Jaecoo 5 EV × Grab SUV เปลี่ยนจาก "รอตรวจ" เป็น "ผ่าน" — `diff_legacy.py` เดิมนับเป็นการ
ถดถอยและทำให้ CI ล้ม ซึ่งแปลว่า **ผลตรวจทุกชิ้นที่นำเข้าจากเว็บแอปจะทำให้ CI ล้ม**
ตอนนี้ช่องที่ข้อสรุปมาจากข้อกล่าวอ้างที่ตรวจหลังวันย้ายระบบถูกรายงานแยกเป็น "อัปเดต"
พร้อมวันที่ ส่วนข้อกล่าวอ้างที่ไม่ได้ใหม่กว่าข้อมูลเดิมแต่เปลี่ยนความหมายยังถูกจับเหมือนเดิม

**ง. สิ่งที่ปรับตามมา**
- `migrate_legacy.py` ไม่ยอมเขียนทับถ้าข้อมูลปัจจุบันมีแถวที่สร้างซ้ำจากข้อมูลเดิมไม่ได้
  (รถที่เพิ่ม · ผลตรวจที่นำเข้า) — เดิมรันซ้ำแล้วลบทิ้งเงียบ ๆ ต้องใช้ `--force` ถ้าตั้งใจ
- หัวเว็บแอปแสดงสัดส่วนข้อสรุปที่เก่าเกินเพดานคู่กับวันที่ตรวจล่าสุด — ผลตรวจใหม่ไม่กี่แถว
  ไม่ทำให้ทั้งหน้าดูสดอีกต่อไป
